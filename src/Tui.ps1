...
function Get-PoshPalettePromptParts {
    ...
    if ($Theme.prompt.generated) {
        switch -Wildcard ($Theme.prompt.name) {
            ...
            # bong: bespoke info line (cloud + date | path + git branch), then a dog + bone prompt. Apricot accents (yellow).
            '*bong*'      { return @($sc.yellow, "$([char]0xF0C2)  ", $sc.foreground, '16/07 Thu 12:27 | ', $sc.yellow, 'C:\proj\ccbit ', $sc.purple, "$([char]0xF418) main ", $sc.yellow, "$([char]0xEEF7) $([char]0xEE9A) ") }
